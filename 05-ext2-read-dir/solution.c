...
int report_direct(int img, const uint32_t block, const size_t block_size, void* buf) {
    ...
    ssize_t left_to_read = (ssize_t) block_size;
    struct ext2_dir_entry_2* dir_entry = NULL;
    off_t offset = 0;
    for (dir_entry = get_dir_entry(buf, 0); dir_entry->inode != 0 && left_to_read > 0; dir_entry = get_dir_entry(buf, offset)) {
        ...
        left_to_read -= dir_entry->rec_len;
...
int report_indirect(int img, const uint32_t block, const size_t block_size, uint32_t* buf, int block_type) {
    ssize_t bytes_read = read_block(img, block, block_size, buf);
    if (bytes_read < (ssize_t) block_size) {
        return -errno;
    }
    ...
    for (size_t i = 0; i < block_size / sizeof(uint32_t) && buf[i] != 0; ++i) {
        if (block_type == EXT2_IND_BLOCK) {
            retval = report_direct(img, buf[i], block_size, indirect_block_buf);
        } else {
            retval = report_indirect(img, buf[i], block_size, (uint32_t*) indirect_block_buf, i - 1);
        }
        if (retval < 0) {
            break;
        }
...
int dump_dir(int img, int inode_nr) {
    ...
    int retval = 0;
    for (size_t i = 0; i < EXT2_N_BLOCKS && inode.i_block[i] != 0; ++i) {
        if (i < EXT2_NDIR_BLOCKS) {
            retval = report_direct(img, inode.i_block[i], block_size, buf);
        } else if (i == EXT2_IND_BLOCK || i == EXT2_DIND_BLOCK || i == EXT2_TIND_BLOCK) {
            retval = report_indirect(img, inode.i_block[i], block_size, (uint32_t*) buf, i);
        } else {
            retval = -1;
        }
        if (retval < 0) {
